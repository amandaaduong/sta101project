...
MI_raw <- read.csv("~/Desktop/mammalianMI-main/data/MI.csv", sep = ";")
species_count <- read.csv("~/Desktop/mammalianMI-main/data/Species_count.csv", sep=";")
names(MI_raw)
names(species_count)
...
the.model1 = lm(Litter_mass ~ Adult_mass, data = MI_full)
MI_full$ei = the.model1$residuals
MI_full$yhat = the.model1$fitted.values

qqnorm(the.model1$residuals)
qqline(the.model1$residuals)

the.model = lm(Litter_mass_log10 ~ Adult_mass_log10, data = MI_full)
MI_full$ei = the.model$residuals
MI_full$yhat = the.model$fitted.values
...
#Residuals
library(ggplot2)
qplot(yhat, ei, data = MI_full) +  ggtitle("Errors vs. Fitted Values") + xlab("Fitted Values") + 
...
residuals_SLR <- residuals(fit_SLR_models)
...
```
```{r}
residuals_PLMM <- residuals(fit_PLMM_models)

# Create a QQ plot of the residuals
qqnorm(residuals_PLMM, main = "QQ Plot of Residuals for PLMM Model")
qqline(residuals_PLMM, col = "red")
```
